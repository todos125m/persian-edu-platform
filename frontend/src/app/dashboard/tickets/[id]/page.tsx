...
export default function TicketDetailPage() {
  ...
  const replyMutation = useMutation({
    mutationFn: () => ticketsService.reply(ticketId, replyText),
    onSuccess: () => {
      setReplyText('');
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={cn(
                      'text-xs font-medium',
                      isAdmin ? 'text-primary-600' : 'text-primary-100'
                    )}
                  >
                    {isAdmin ? 'پشتیبانی' : `${msg.user.firstName} ${msg.user.lastName}`}
                  </span>
                  <span
                    className={cn(
                      'text-xs',
      ...
      {!isClosed ? (
        ...
        <div className="mt-3 text-center py-3 bg-gray-100 rounded-xl text-sm text-gray-500">
          این تیکت بسته شده و امکان پاسخ‌دهی ندارد
        </div>
      )}
    </div>
  );
